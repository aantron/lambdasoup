(* This file is a modified version of netstring/nethtml.ml from the Ocamlnet
   project. See LICENSE. Ocamlnet can be found at
   http://projects.camlcity.org/projects/ocamlnet.html. *)

open Soup_ocamlnet_nethtml_scanner;;
...
let parse_document ?(dtd = html40_dtd) 
  ...
  in
  try
    ignore (parse_next());  (* never returns. Will get a warning X *)
    assert false
  with
      End_of_scan ->
